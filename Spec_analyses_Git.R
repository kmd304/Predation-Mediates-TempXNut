...
full.model.1 <- lm(Spec~Temp*Nutrient*Protist, na.action = "na.fail", data= master)
summary(full.model.1)
boxplot((Spec)~Temp*Nutrient*Protist, data = master)
boxplot((Spec)~Nutrient*Protist, data = master)

# Calculate Z-scores
sem.new$Spec1_z <- scale(sem.new$Spec1)
sem.new$biomass.prot1_z <- scale(sem.new$biomass.pro1)
...
pdf(file = "C:/Users/18568/Box/Duke/Fig1.pdf", width = 20, height = 10)
